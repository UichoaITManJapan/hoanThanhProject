...
public class Category implements IEntity, Serializable {
    ...
    public int validateCatalogId(){
        System.out.println(ColorManager.PINK+"Nhập vào mã thể loại :"+ColorManager.RESET);
        do {
            ...
            if (inputStringCatalogId != null || inputStringCatalogId.trim().length() != 0){
                ...
                System.out.println(ColorManager.RED+"Mã thể loại không được để trống. Vui lòng nhập mã thể loại."+ColorManager.RESET);
            }
        } while (true);
    ...
    public static String validateCatalogName(){
        System.out.println(ColorManager.PINK+"Nhập vào tên thể loại :"+ColorManager.RESET);
        do {
                ...
                try {
                    ...
                    for (Category ct: categoryList) {
                        if (ct.getName().length() >= 6 && ct.getName().length()<= 30){
                            ...
                            System.out.println(ColorManager.RED+"Tên thể loại phải có từ 6-30 ký tự. Vui lòng nhập lại"+ColorManager.RESET);
                        }
                    }
    ...
    public static boolean validateCatalogStatus(){
        System.out.println(ColorManager.PINK+"Nhập vào trạng thái thể loại :"+ColorManager.RESET);
        do {
            ...
            if (!inputStatus.isEmpty()){
                if (inputStatus.equals("true")||inputStatus.equals("false")){
                    ...
                    System.out.println(ColorManager.RED+"Đầu vào không hợp tt. Vui lòng nhập trạng thái thể loại là 'true' hoặc 'false'!"+ColorManager.RESET);
                }
            } else {
    ...
    @Override
    public void output() {
        String checkStatus = this.status ? ColorManager.GREEN+"Hoạt động      "+ColorManager.RESET : ColorManager.RED+"Không hoạt động"+ColorManager.RESET;
//        DecimalFormat ft = new DecimalFormat("0000");
        System.out.printf(ColorManager.BLUE+"◆"+ColorManager.VANGDAM+" \t%d"+ColorManager.RESET+ColorManager.PURPLE+" \t\t %s"+ColorManager.RESET+" \t\t  %s\t\t "+ColorManager.BLUE+"◆\n",id,name,checkStatus);
